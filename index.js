 ...
 * version: 2026-02-28-webapp-collections-subs-dark-v7
 */
...
// ============ init ============
console.log('v7 start', { MODE, WEBAPP_URL: !!WEBAPP_URL, REDIS: !!REDIS_URL, ADMIN_USER_ID, MRKT_AUTH: !!MRKT_AUTH_RUNTIME });
...
function formatPctFromPermille(p){
  const v = Number(p);
  if(!Number.isFinite(v)) return null;
  const pct = v/10;
...
async function sendMessageSafe(chatId, text, opts){
  while(true){
    try { return await bot.sendMessage(chatId, text, opts); }
      ...
      const ra = e?.response?.body?.parameters?.retry_after ?? e?.response?.parameters?.retry_after ?? null;
      if(ra){ await sleep((Number(ra)+1)*1000); continue; }
      throw e;
    }
  }
}
...
async function mrktGetCollections(){
  ...
  if(!res || !res.ok){
    ...
    const fallback = MRKT_COLLECTIONS.map(x => ({ name:x, title:x, modelStickerThumbnailKey:null, floorPriceNanoTons:null, isHidden:false }));
  ...
  // оставляем только не скрытые
  const items = arr.filter(x => x && x.isHidden !== true).map(x => ({
...
async function mrktGetModelsForGift(giftName){
  ...
  for(const it of arr){
    ...
    const key = norm(name);
    if(!map.has(key)){
      map.set(key, { name:String(name), rarityPerMille: it.rarityPerMille ?? null, thumbKey: it.modelStickerThumbnailKey || null });
    }
  }
...
async function mrktGetBackdropsForGift(giftName){
  ...
  for(const it of arr){
    ...
    const key = norm(name);
    if(!map.has(key)){
      map.set(key, { name:String(name), rarityPerMille: it.rarityPerMille ?? null, centerHex: intToHexColor(it.colorsCenterColor) });
    }
  }
  const items = Array.from(map.values());
  backdropsCache.set(giftName, { time: nowMs(), items });
      ...
      for(const sub of active){
        ...
        if(canNotify && prevFloor!==newFloor && newFloor!=null){
          await sendMessageSafe(userId,
            ...
            { disable_web_page_preview:true, reply_markup: lot?.urlMarket ? { inline_keyboard:[[{text:'Открыть MRKT', url: lot.urlMarket}]] } : undefined }
          );
          floorNotifs++;
        }
    ...
    for(const [userId, user] of users.entries()){
      ...
      for(const g of list){
        ...
        await sendMessageSafe(userId,
          ...
          { disable_web_page_preview:false, reply_markup:{ inline_keyboard:[[{text:'Открыть MRKT', url:g.urlMarket}]] } }
        );
...
const WEBAPP_JS = `(() => {
  ...
  // If opened outside Telegram WebApp
  if (!initData) {
  ...
  let selectedGiftValue = ''; // collection "name" to send to API
  ...
  // Gift suggest from /gifts/collections
...
function startWebServer(){
  ...
  app.post('/api/admin/mrkt_auth', auth, async (req,res)=>{
    ...
    modelsCache.clear(); backdropsCache.clear();
    res.json({ok:true});
  });
...
// ============ intervals ============
setInterval(()=>{ checkMarketsForAllUsers().catch(()=>{}); }, CHECK_INTERVAL_MS);
setInterval(()=>{ checkSubscriptionsForAllUsers().catch(()=>{}); }, SUBS_CHECK_INTERVAL_MS);

...
const PORTAL_THROTTLE_MS = Number(process.env.PORTAL_THROTTLE_MS || 500);
...
const TONNEL_AUTHDATA = process.env.TONNEL_AUTHDATA || TONNEL_USER_AUTH;
...
const TONNEL_THROTTLE_MS = Number(process.env.TONNEL_THROTTLE_MS || 800);
let tonnelNextAllowedAt = 0;

console.log('Bot version 2026-02-21-full-headers-proxy');
...
console.log('HTTP_PROXY =', HTTP_PROXY ? 'set' : 'not set');
...
const catalog = {
  gifts: new Map(),
  modelsByGift: new Map(),
  backdropsByGift: new Map(),
};
...
function catalogExport() {
  ...
  for (const [g, map] of catalog.modelsByGift.entries()) {
    modelsByGift[g] = Array.from(map.entries());
  }
...
async function throttledPortalFetch(url, opts) {
  ...
  for (let i = 0; i < 3; i++) {
    ...
    if (setCookie) portalCookies = setCookie; // упрощённо
...
function tonnelHeaders(extra = {}) {
  return {
    ...
    ...(tonnelCookies ? { Cookie: tonnelCookies } : {}),
    ...extra,
  };
}
...
async function throttledTonnelFetch(url, opts) {
  ...
  const fetchOpts = {
    ...opts,
    headers: tonnelHeaders(opts.headers),
    ...(proxyAgent ? { agent: proxyAgent } : {}),
  };
  ...
  if (!res) return null;
  const setCookie = res.headers.get('set-cookie');
  if (setCookie) tonnelCookies = setCookie;
  return res;
}
...
async function portalProbe() {
  ...
  const res = await throttledPortalFetch(url, { method: 'GET' }).catch(() => null);
...
async function tonnelProbe() {
  ...
  const res = await throttledTonnelFetch(url, { method: 'POST', body: JSON.stringify(body) }).catch(() => null);
  if (!res) return { ok: false, status: null, note: 'fetch error', bodyStart: '' };
  const ct = res.headers.get('content-type') || '';
  const txt = await res.text().catch(() => '');
  ...
  console.log(`Tonnel probe: status=${res.status}, bodyStart=${txt.slice(0,100)}`);
  return { ok: res.ok, status: res.status, contentType: ct, bodyStart: txt.slice(0, 220) };
}

// =====================
// MRKT auth (остаётся без изменений)
// =====================
...
async function notifyMrktAuthExpired(statusCode) {
  ...
  const text = `⚠️ MRKT токен не работает (HTTP ${statusCode}).\nНужно обновить переменную MRKT_AUTH.`;
  if (ADMIN_CHAT_ID && Number.isFinite(ADMIN_CHAT_ID)) {
    try { await sendMessageSafe(ADMIN_CHAT_ID, text, { disable_web_page_preview: true }); } catch {}
  } else {
...
// =====================
// Portal collections / filters (используют throttledPortalFetch)
// =====================
...
function extractRarityValue(obj) {
  ...
  const direct = obj.rarityPermille ?? obj.rarity_per_mille ?? obj.rarityPerMille ?? obj.rarity ?? obj.rarity_percent ?? obj.rarityPercent ?? null;
...
// =====================
// Portal search (использует throttledPortalFetch)
// =====================
...
async function portalSearchByFilters({ giftLower, modelLower, backdropLower, minPriceTon, maxPriceTon }) {
  ...
  for (let page = 0; page < PORTAL_PAGES; page++) {
    ...
    const r = await portalSearchPage({ giftLower, modelLower, backdropLower, minPriceTon, maxPriceTon, offset, limit: PORTAL_LIMIT });
    if (!r.ok) return r;
...
// =====================
// Portal history (без изменений, использует throttledPortalFetch)
// =====================
function extractActionNft(act) {
  return act?.nft || act?.item?.nft || act?.gift || act?.asset || act?.data?.nft || act?.data?.gift || null;
}
...
async function portalHistoryEstimate({ collectionId, modelLower, backdropLower }) {
  ...
  while (page < PORTAL_HISTORY_PAGES && prices.length < PORTAL_HISTORY_TARGET_SALES) {
    ...
    if (modelLower) url += `&filter_by_models=${quotePlus(capWords(modelLower))}`;
    if (backdropLower) url += `&filter_by_backdrops=${quotePlus(capWords(backdropLower))}`;
    let res = await throttledPortalFetch(url, { method: 'GET' }).catch(() => null);
    let serverFiltered = true;
    if (res && !res.ok && res.status === 422) {
      serverFiltered = false;
      url = `${API_URL}market/actions/?offset=${offset}&limit=${PORTAL_HISTORY_LIMIT}&action_types=buy&sort_by=created_at+desc`;
      if (collectionId) url += `&collection_ids=${encodeURIComponent(collectionId)}`;
      res = await throttledPortalFetch(url, { method: 'GET' }).catch(() => null);
    }
    if (!res || !res.ok) break;
    const data = await res.json().catch(() => null);
    const actions = Array.isArray(data?.actions) ? data.actions : Array.isArray(data?.items) ? data.items : Array.isArray(data) ? data : [];
    ...
    for (const act of actions) {
      ...
      if (!serverFiltered) {
        ...
        if (!nft) continue;
        if (collectionId) {
          const actCollectionId = nft?.collection_id || nft?.collectionId || nft?.collection?.id || nft?.collection?.collection_id || null;
...
// =====================
// MRKT functions (без изменений, кроме использования прокси)
// =====================
async function mrktFetchSalingPage({ collectionName, modelName, backdropName, cursor }) {
  ...
  const res = await fetch(`${MRKT_API_URL}/gifts/saling`, fetchOpts).catch(() => null);
...
async function mrktFeedFetch({ collectionName, modelName, backdropName, cursor, count, ordering = 'Latest', types = [] }) {
  ...
  const res = await fetch(`${MRKT_API_URL}/feed`, fetchOpts).catch(() => null);
...
// =====================
// Tonnel functions (используют throttledTonnelFetch)
// =====================
...
async function tonnelSearchByFilters({ giftLower, modelLower, backdropLower }, minPriceTonLocal, maxPriceTonLocal) {
  ...
  for (let p = 1; p <= TONNEL_PAGES; p++) {
    ...
    for (const it of r.items) {
      ...
      const urlMarket = 'https://t.me/Tonnel_Network_bot';
      out.push({
...
async function sendDeal(userId, gift) {
  ...
  const btnText = gift.market === 'Portal' ? 'Открыть Portal' : gift.market === 'MRKT' ? 'Открыть MRKT' : 'Открыть Tonnel';
...
async function sendSellPriceForUser(chatId, user) {
  ...
  const giftName = catalog.gifts.get(giftLower) || collectionsCache.byLowerName.get(giftLower)?.name || capWords(giftLower);
  ...
  if ((user.filters.markets || []).includes('Portal')) {
    ...
    if (r.ok && r.gifts.length) {
      ...
      text += `Portal:\n  ~${best.priceTon.toFixed(3)} TON (флор)\n  Чистыми после комиссии ${(PORTAL_FEE * 100).toFixed(1)}%: ~${net.toFixed(3)} TON\n`;
    } else if (r.ok) {
  ...
  if ((user.filters.markets || []).includes('MRKT')) {
    ...
    if (r.ok && r.gifts.length) {
      ...
      text += `\nMRKT:\n  ~${best.priceTon.toFixed(3)} TON (флор)\n  Комиссия ${(MRKT_FEE * 100).toFixed(1)}%: ~${net.toFixed(3)} TON чистыми\n`;
    } else if (r.ok) {
...
// =====================
// Subscriptions (без изменений, кроме прокси)
// =====================
...
bot.on('message', async (msg) => {
  ...
  if (user.state === 'awaiting_gift_search') {
    ...
    if (!matched.length) {
      return sendMessageSafe(chatId, 'Ничего не нашёл по этому запросу.\nЕсли Portal заблокирован — напиши название подарка вручную (я запомню).', { reply_markup: MAIN_KEYBOARD });
    }
  ...
  if (t === '🎛 Фильтры') {
    const inlineKeyboard = {
      inline_keyboard: [
        ...
        [{ text: '🎯 Выбрать модель', callback_data: 'filter_model' }, { text: '🎨 Выбрать фон', callback_data: 'filter_backdrop' }],
        [{ text: '✍️ Ввести подарок вручную', callback_data: 'gift_free' }, { text: '✍️ Ввести модель вручную', callback_data: 'model_free' }, { text: '✍️ Ввести фон вручную', callback_data: 'backdrop_free' }],
        [{ text: '🔍 Поиск подарка', callback_data: 'search_gift' }, { text: '🔍 Поиск модели', callback_data: 'search_model' }, { text: '🔍 Поиск фона', callback_data: 'search_backdrop' }],
        [{ text: '🅿 Portal', callback_data: 'set_markets:Portal' }, { text: '🅼 MRKT', callback_data: 'set_markets:MRKT' }, ...(TONNEL_ENABLED ? [{ text: '🅣 Tonnel', callback_data: 'set_markets:Tonnel' }] : [])],
        [{ text: '🅼+🅣 MRKT+Tonnel', callback_data: 'set_markets:MRKT_TONNEL' }, { text: '🅿+🅼 Portal+MRKT', callback_data: 'set_markets:PORTAL_MRKT' }, { text: '🅿+🅼+🅣 ALL', callback_data: 'set_markets:ALL' }],
        [{ text: '♻️ Сбросить модель', callback_data: 'clear_model' }, { text: '♻️ Сбросить фон', callback_data: 'clear_backdrop' }],
...
bot.on('callback_query', async (query) => {
  ...
  try {
    if (data === 'gift_free') {
      ...
      if (!names.length) {
        user.state = 'awaiting_gift_free'; scheduleSave();
        await sendMessageSafe(chatId, 'Список подарков сейчас недоступен (Portal может быть заблокирован Cloudflare).\nНапиши название подарка вручную:', { reply_markup: MAIN_KEYBOARD });
      } else {
      ...
      await sendMessageSafe(chatId, `Проверка подписок завершена.\n• Подписок проверено: ${st.processedSubs}\n• Уведомлений (флор): ${st.floorNotifs}\n• Уведомлений (MRKT feed): ${st.feedNotifs}`, { reply_markup: MAIN_KEYBOARD });

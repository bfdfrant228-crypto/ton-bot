(() => {
  ...
  if (saveBtn) {
    saveBtn.onclick = wrap('subs', async()=>{
      const data = el('mrktData').value.trim();
      const photo = el('mrktPhoto').value.trim();
      await api('/api/admin/mrkt_session', { method:'POST', body: JSON.stringify({ data, photo }) });
      await refreshAdmin();
    });
  }

  if (forceBtn) {
    forceBtn.onclick = wrap('subs', async()=>{
